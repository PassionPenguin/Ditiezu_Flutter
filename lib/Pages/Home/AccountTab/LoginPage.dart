...
class _LoginPageState extends State<LoginPage> {
  ...
  _init() {
    () async {
      if (await NetWork().checkLogin()) {
        Toast(context, "正在跳转中", accentColor: Colors.lightGreen, icon: Icons.check);
        isLoading = false;
        setState(() {});
        ...
        child: Container(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: <Widget>[
            ...
            Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: [
              SizedBox(height: 24),
              TextInput(_usrNameController, TextInputType.text, prefix: Row(children: [Padding(padding: EdgeInsets.only(left: 12)), Icon(Icons.person, color: Colors.grey, size: 16)]), placeholder: "用户名", style: TextStyle(fontSize: 14)),
              SizedBox(height: 24),
              TextInput(_pwdController, TextInputType.visiblePassword,
                  prefix: Row(children: [Padding(padding: EdgeInsets.only(left: 12)), Icon(Icons.lock, color: Colors.grey, size: 16)]), placeholder: "密码", style: TextStyle(fontSize: 14), obscureText: true),
              SizedBox(height: 24),
              Row(children: [
                Container(
                  width: 60,
                ...
                Expanded(child: TextInput(_codeController, TextInputType.text, prefix: Row(children: [Padding(padding: EdgeInsets.only(left: 12)), Icon(Icons.code, color: Colors.grey, size: 16)]), placeholder: "验证码", style: TextStyle(fontSize: 14)))
              ]),
              SizedBox(height: 12),
              Row(children: [
                Expanded(
              ...
              Row(children: [
                Expanded(
                    child: TextInput(_questionController, TextInputType.text,
                        prefix: Row(children: [Padding(padding: EdgeInsets.only(left: 12)), Icon(Icons.question_answer, color: Colors.grey, size: 16)]), placeholder: "答案", style: TextStyle(fontSize: 14)))
              ]),
              SizedBox(height: 12),
              Row(children: [
                Expanded(
                  ...
                  child: Wrap(children: [
                    Offstage(
                        offstage: isLoading,
                        child:
                            AnimatedOpacity(opacity: isLoading ? 0 : 1, duration: Duration(seconds: 1), child: SizedBox(height: 20, child: Text("登录", style: TextStyle(color: Colors.black, fontSize: 18, height: 1.15, fontWeight: FontWeight.w600))))),
                    Offstage(
                        offstage: !isLoading,
  ...
  void performLogin() async {
    ...
    Toast(context, doc.querySelector("#messagetext .mbn").text, accentColor: status ? Colors.lightGreen : Colors.red, icon: status ? Icons.check : Icons.clear);
    if (status) {
      var usr = User(
    ...
    if (doc.querySelector("#messagetext :first-child").innerHtml.contains("验证码填写错误")) {
      _init();
    }
    isLoading = false;
    setState(() {});
