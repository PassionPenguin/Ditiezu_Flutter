...
class _LoginPageState extends State<LoginPage> {
  ...
  Future<NetworkImage> loadCode() async {
    ...
    var response = await NetWork().get(
        "http://www.ditiezu.com/misc.php?mod=seccode&action=update&idhash=$codeHash&inajax=1&ajaxtarget=seccode_");
    var doc = parseHtmlDocument(
        parseXmlDocument(response).firstChild.text); // <CDATA>
    return NetworkImage(
        "http://www.ditiezu.com/" + doc.querySelector("img").attributes["src"],
        headers: {
          "user-agent":
            ...
            Column(
                ...
                children: [
                  ...
                  Row(children: [
                    Container(
                      ...
                      child: Row(
                        ...
                        children: [
                          Image(
                              image: codeImage, width: 48, fit: BoxFit.contain),
                        ],
                      ),
                    ),
                    Expanded(
                  ...
                  Row(children: [
                    Expanded(
                        child:
                            TextInput(_questionController, TextInputType.text,
                                prefix: Row(children: [
                                  Padding(padding: EdgeInsets.only(left: 12)),
    ...
    if (status) {
      var usr = User(
          uid: int.parse(response.substring(
              response.indexOf("discuz_uid = '") + 14,
              response.indexOf("'", response.indexOf("discuz_uid = '") + 14))),
          userName: response.substring(response.indexOf("欢迎您回来，") + 6,
              response.indexOf("。", response.indexOf("欢迎您回来，"))),
          loginState: true);
