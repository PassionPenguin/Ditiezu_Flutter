...
class _LoginPageState extends State<LoginPage> {
  ...
  Future<NetworkImage> loadCode() async {
    ...
    return NetworkImage("http://www.ditiezu.com/" + doc.querySelector("img").attributes["src"],
        headers: {
          "user-agent":
          ...
          "cookie":
              PersistCookieJar(dir: (await getApplicationDocumentsDirectory()).path + "/.cookies/")
  ...
  void initState() {
    ...
    () async {
      ...
      if (cookieJar.loadForRequest(Uri.http("www.ditiezu", "")).isEmpty)
        await NetWork(retrieveAsDesktopPage: false, gbkDecoding: false)
            .get("http://www.ditiezu.com/forum.php?mod=forum");
      var response =
          await NetWork(retrieveAsDesktopPage: false, gbkDecoding: false, autoRedirect: false)
              .get("http://www.ditiezu.com/member.php?mod=logging&action=login&mobile=yes");
      print(response);
                  ...
                  Row(children: [
                    Expanded(
                        child: TextInput(_questionController, TextInputType.text,
                            prefix: Row(children: [
                              Padding(padding: EdgeInsets.only(left: 12)),
                              Icon(Icons.question_answer, color: Colors.grey, size: 16)
                            ]),
                            placeholder: "Answer",
                            style: TextStyle(fontSize: 14),
                            obscureText: true))
