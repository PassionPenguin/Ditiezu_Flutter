import 'package:Ditiezu/Models/SelectItem.dart';
import 'package:flutter/material.dart';
import 'package:flutter/widgets.dart';

import 'drapdown_common.dart';

typedef Widget MenuItemBuilder<T>(BuildContext context, SelectItem data, bool selected);
...
class DropdownListMenu<T> extends DropdownWidget {
  ...
  DropdownListMenu({this.data, this.selectedIndex, this.itemBuilder, this.itemExtent: kDropdownMenuItemHeight, this.callback});
...
class _MenuListState<T> extends DropdownState<DropdownListMenu<T>> {
  ...
  Widget buildItem(BuildContext context, int index) {
    final List<SelectItem> list = widget.data;

    final SelectItem data = list[index];
...
class _TreeMenuList<T, E> extends DropdownState<DropdownTreeMenu> {
  ...
  List<SelectItem> _subData;

  List<SelectItem> _data;
  ...
  Widget buildSubItem(BuildContext context, int index) {
    return new GestureDetector(
      behavior: HitTestBehavior.opaque,
      child: widget.subItemBuilder(context, _subData[index], _activeIndex == _selectedIndex && index == _subSelectedIndex),
      onTap: () {
        assert(controller != null);
        controller.select(_subData[index], index: _activeIndex, subIndex: index);
        setState(() {
