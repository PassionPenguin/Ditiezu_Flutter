...
class Asset {
  ...
  String get _channel {
    return 'image_picker/image/$_identifier';
  }
  ...
  Future<ByteData> getThumbByteData(int width, int height,
    ...
    await ImagePicker.requestThumbnail(
  ...
  Future<ByteData> getByteData({int quality = 100}) async {
    ...
    await ImagePicker.requestOriginal(_identifier, quality);
    return completer.future;
  }

  /// Requests the original image meta data
  Future<Metadata> get metadata {
    return ImagePicker.requestMetadata(_identifier);
  }
