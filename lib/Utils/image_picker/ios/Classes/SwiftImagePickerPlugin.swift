...
public class SwiftImagePickerPlugin: NSObject, FlutterPlugin {
    ...
    public static func register(with registrar: FlutterPluginRegistrar) {
        ...
        let instance = SwiftImagePickerPlugin.init(messenger: registrar.messenger())
            ...
            if (assets.count > 0) {
                ...
                let ID: PHImageRequestID = manager.requestImage(
                    ...
                    resultHandler: {
                        ...
                        self.messenger.send(onChannel: "image_picker/image/" + identifier + ".thumb", message: image?.jpegData(compressionQuality: CGFloat(compressionQuality)))
                        })
        ...
        case "requestOriginal":
            ...
            if (assets.count > 0) {
                ...
                let ID: PHImageRequestID = manager.requestImage(
                    ...
                    resultHandler: {
                        (image: UIImage?, info) in
                        self.messenger.send(onChannel: "image_picker/image/" + identifier + ".original", message: image!.jpegData(compressionQuality: CGFloat(compressionQuality)))
                })
