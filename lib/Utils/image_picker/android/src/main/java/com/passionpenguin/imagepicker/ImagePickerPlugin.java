 ...
 * ImagePickerPlugin
 */
public class ImagePickerPlugin implements
        FlutterPlugin,
        ActivityAware,
        MethodCallHandler,
    ...
    public static void registerWith(Registrar registrar) {
        ImagePickerPlugin instance = new ImagePickerPlugin();
    ...
    private static class GetThumbnailTask extends AsyncTask<String, Void, ByteBuffer> {
        ...
        protected void onPostExecute(ByteBuffer buffer) {
            ...
            if (buffer != null) {
                this.messenger.send("image_picker/image/" + this.identifier + ".thumb", buffer);
                buffer.clear();
            }
        }
    }
    ...
    private static class GetImageTask extends AsyncTask<String, Void, ByteBuffer> {
        ...
        protected void onPostExecute(ByteBuffer buffer) {
            super.onPostExecute(buffer);
            this.messenger.send("image_picker/image/" + this.identifier + ".original", buffer);
            buffer.clear();
        }
    }
    ...
    private void presentPicker(int maxImages, boolean enableCamera, ArrayList<String> selectedAssets, HashMap<String, String> options) {
        ...
        FishBun.with(ImagePickerPlugin.this.activity)
                .setImageAdapter(new GlideAdapter())
                .setSelectedImages(selectedUris)
                .setMaxCount(9)
                .setPickerSpanCount(3)
                .setActionBarColor(Color.parseColor("#4FC3F7"), Color.parseColor("#4FC3F7"), true)
                ...
                .setAllViewTitle("全部图片")
                .setActionBarTitle("选择图片")
                .textOnImagesSelectionLimitReached("您已不能选择更多图片")
                .textOnNothingSelected("至少需要一张图片")
